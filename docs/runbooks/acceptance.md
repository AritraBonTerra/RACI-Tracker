...
| 7 | Chain Plan Member sees phases 1–3 and every promotion under the plan, including later ones | `scopedReads` "a Chain Plan Member sees the plan and every promotion under it", "records created after a grant are inside it" |
...
| 9 | Every reference-data, plan-year or deletion write by a Member is refused server-side; opening a chain plan or promotion follows the grant | `scopedWrites` "hierarchy, reference-data and People writes are refused for every Member", "an Editor opens chain plans and promotions exactly where a year or chain grant reaches"; `chainAccess` "a chain Editor starts the chain's plan in a new year and opens promotions under it" |
